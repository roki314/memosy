...
function createMemo(id) {
  ...
  memo.innerHTML = `
    ...
    <div class="color-picker">
      ${['#FFD700', '#FFB6C1', '#90EE90', '#87CEFA', '#FFA07A', '#DDA0DD', '#bbb', '#999', '#777', '#555']
          .map(color1 => `
        <div class="color-swatch" style="background-color: ${color1}" data-color="${color1}">O</div>
      `).join('')}    
    </div>
...
document.addEventListener('touchstart', e => {
  ...
  if (memo && !e.target.isContentEditable && !e.target.classList.contains('color-swatch')) {
    activeMemo = memo;
    offsetX = touch.clientX - memo.offsetLeft;
    offsetY = touch.clientY - memo.offsetTop;
    memo.style.zIndex = 1000;
  }
});
...
document.addEventListener('touchmove', e => {
  if (activeMemo) {
    const touch = e.touches[0];
    activeMemo.style.left = `${touch.clientX - offsetX}px`;
    activeMemo.style.top = `${touch.clientY - offsetY}px`;
  }
});
